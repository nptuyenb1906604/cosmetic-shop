            ...
            <div class="row">
                ...
                <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                    <div class="login-box">
                        ...
                        ?>
                                <a id="basic" class="selectpicker show-tick form-control" href="{{URL::to('/login')}}"><i class="fa-solid fa-lock"></i> Đăng nhập</a>
                        <?php
                            }
                        ?>
                    </div>
                    <div class="text-slid-box">
                        <div id="offer-box" class="carouselTicker">
                            <ul class="offer-box">
                                <li>
                                    <i class="fab fa-opencart"></i> Giảm 20% cho toàn bộ đơn hàng. Mã khuyến mãi: laviT80
                                </li>
                                <li>
                    ...
                    <ul class="nav navbar-nav ml-auto" data-in="fadeInDown" data-out="fadeOutUp">
                        ...
                        <li class="dropdown">
                            ...
                            <ul class="dropdown-menu">
                                ...
                                <li><a href="{{URL::to('/show_cart_ajax')}}">Giỏ hàng</a></li>
                ...
                <li class="cart-box">
                    <ul class="cart-list">
                        ...
                        @if(Session::get('cart'))
                            ...
                            <li>
                                <a href="#" class="photo"><img src="{{URL::to('public/upload_image/product/'.$value['product_image'])}}" class="cart-thumb" alt="" /></a>
                                <h6><a href="#">{{$value['product_name']}}</a></h6>
                                <p><span>{{$value['product_qty']}}<span>x - <span class="price">{{number_format($value['product_price']).' VNĐ'}}</span></p>
                            </li>
                            @endforeach
                            <li class="total">
                                ...
                                <a href="{{URL::to('/show_cart_ajax')}}" class="btn btn-default hvr-hover btn-cart">Xem Giỏ Hàng</a>
                            </li>
                        @else
                            <p style="background-color: black; " class="text-white text-center">Hãy thêm sản phẩm vào giỏ hàng</p>
                        @endif
                    </ul>
                </li>
            </div>
    ...
    <script type="text/javascript">
       $(document).ready(function(){
            $('.calculate_delivery').click(function(){
                ...
                var _token = $('input[name="_token"]').val();
                if(matp == '' && maqh == '' && xaid == ''){
                    alert('Xin hãy chọn để tính phí vận chuyển');
                }else{
                    $.ajax({
                        ...
                        success:function(data){
                            window.location.href = "{{url('/show_checkout')}}";
                        }
                    });
                }
            });
        ...
        $(document).ready(function(){
            $('.send_order').click(function(){
                swal({
                        ...
                        text: "Không thể hủy đơn hàng đã đặt, bạn có muốn đặt hàng không?",
                        type: "warning",
                        showCancelButton: true,
                        cancelButtonText: "Chưa muốn mua",
                    ...
                    function(isConfirm){
                        if(isConfirm){
                            ...
                            $.ajax({
                                    ...
                                    method:'POST',
                                    data:{shipping_name:shipping_name,shipping_email:shipping_email,shipping_address:shipping_address,shipping_phone:shipping_phone,shipping_note:shipping_note,shipping_payment:shipping_payment,order_fee:order_fee,order_matp:order_matp,order_maqh:order_maqh,order_xaid:order_xaid,_token:_token},
                                    success:function(data){
                                        swal("Đơn hàng", "Đơn hàng của bạn được đặt thành công", "success");
                                    }
                                });

                            window.setTimeout(function(){ //3s reset lại form checkout, ko cần click ok
                                window.location.href = "{{url('/')}}";
                            }, 3000);
                            
                        }else{
                            swal("Đóng", "Đặt hàng không thành công", "error");
                        }
                    }
                );
            });
